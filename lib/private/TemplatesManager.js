...
TemplatesManager.prototype = {
     ...
     */
    updateEachHTMLFiles: function( patterns ){
        var deferred = Q.defer(),
            globOpts = {},
            ignorePatterns = [],
            updateHTMLFile = this.updateHTMLFile.bind( this );
        ...
        if( _.isArray( patterns ) ){
            ...
            patterns = ( patterns.length > 1 ) ?
                ["{", patterns.join( "," ), "}"].join( "" ): patterns[0]

            if( ignorePatterns.length ){
                globOpts.ignore = ( ignorePatterns.length > 1 ) ?
                    ["{", ignorePatterns.join( "," ), "}"].join( "" ): ignorePatterns[0];
            }
        }

        glob( patterns, globOpts, function( err, files ){
            files.reduce( function( previousUpdating, currentTargetFilePath ){
                return previousUpdating.then( function(){ return updateHTMLFile( currentTargetFilePath ); } );
            }.bind( this ), Q() )
