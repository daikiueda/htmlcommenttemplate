...
TargetHTML.prototype = {
    ...
    pickOutValues: function(){
        var values = {},
            $ = cheerio.load( this.activateInstanceTags( this.srcHTMLCode ), { decodeEntities: false } ),

            templateAttr = this.srcHTMLCode.match( /<!-- InstanceBegin template="[^"]+"([^>]*) -->/i );

        if( templateAttr && templateAttr[ 1 ] !== "" ){
