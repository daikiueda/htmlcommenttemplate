...
Template.prototype = {
    ...
    pickOutDefaultValues: function( HTMLCode ){
        var values = {},
            $parent = cheerio.load( utils.activateInstanceTags( HTMLCode ), { decodeEntities: false } ),
        ...
        [
            ...
            .map( function( testSet ){
                ...
                selectors.map( function( selector ){
                    $doc( selector ).each( function( index, element ){
                        values[ $doc( element ).attr( "name" ) ] =
                            this.convertResourcePathFormat( $doc( element ).html() );
                    }.bind( this ) );
                }, this );
            }, this );

        // <!-- InstanceBegin template --> の属性を取得
        templateAttr = HTMLCode.match( /<!-- InstanceBegin template="[^"]+"([^>]*) -->/i );
        if( templateAttr ){
            values[ "__template_attr__" ] = templateAttr[ 1 ];
        }
    ...
    convertToTemplateFormat: function( HTMLCode ){
        ...
        // このテンプレートから生成されるHTMLコードに対し、出自を示すタグ<!-- InstanceBegin template -->を付与する。
        // また、このタグには、任意の属性を適用できるようプレースホルダ（__template_attr__）を設置する。
        // なお、このテンプレートが親テンプレートを持つ場合、すでに<!-- InstanceBegin 〜 -->記述を持つ可能性がある。
        // そのため、変数processedHTMLCodeの定義時において、あらかじめ<!-- InstanceBegin 〜 -->を削除している。
        $( "html" )
            .prepend( [
                "<!--",
                " InstanceBegin template=\"/" + this.id + "\"",
                "<%=",
                " typeof __template_attr__ !== 'undefined' ? __template_attr__: ",
                " typeof __default__.__template_attr__ !== 'undefined' ? __default__.__template_attr__: '' ",
                " %>",
                " -->"
            ].join( "" ) )
            .append( "<!-- InstanceEnd -->" );
        ...
        _.forEach(
            ...
            function( attr, tagName ){
                $( tagName + "[" + attr + "]" ).each( function( index, elm ){
                    ...
                    if( testValue.match( /^(#|\/|http:\/\/|https:\/\/|tel:|mailto:)/ ) ){ return; }
