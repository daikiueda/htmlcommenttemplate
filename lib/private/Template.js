...
Template.prototype = {
    ...
    pickOutDefaultValues: function( HTMLCode ){
        var values = {},
            $inherit = cheerio.load( utils.activateInstanceTags( HTMLCode ) ),
            $self = cheerio.load( utils.activateTemplateTags( HTMLCode ) );

        // まず、親テンプレートから継承したプレースホルダから値を抽出する。
        $inherit( "InstanceEditable" ).each( function( index, element ){
            values[ $inherit( element ).attr( "name" ) ] =
                utils.decodeText( this.convertResourcePathFormat( $inherit( element ).html() ) );
        }.bind( this ) );
        ...
        $self( "TemplateEditable" ).each( function( index, element ){
            values[ $self( element ).attr( "name" ) ] =
                utils.decodeText( this.convertResourcePathFormat( $self( element ).html() ) );
        }.bind( this ) );
    ...
    convertToTemplateFormat: function( HTMLCode ){
        ...
        var processedHTMLCode =
                HTMLCode
                    .replace( /<!-- InstanceBegin template[^>]+ -->/ig, "" )
                    .replace( "<!-- InstanceEnd -->", "" ),
            $ = cheerio.load( utils.activateInstanceTags( processedHTMLCode ) );

        $( "html" )
            .prepend( "<!-- InstanceBegin template=\"/<%- __templateId__ %>\" -->" )
            .append( "<!-- InstanceEnd -->" );
        ...
        processedHTMLCode = utils.decodeText( $.html() );

        processedHTMLCode = _.reduce(
            {
            ...
            this.convertResourcePathFormat( processedHTMLCode )
        );
    ...
    convertResourcePathFormat: function( HTMLCode ){
        ...
        return _.reduce(
            {
                "=\"&lt;%- __normalizePath__\\( &quot;(.+)&quot; \\) %&gt;\"": "=\"<%- __normalizePath__( \"$1\" ) %>\""
            },
            function( replacedCode, replacement, pattern ){
                return replacedCode.replace( new RegExp( pattern, "gi" ), replacement, "gi" );
            },
            utils.decodeText( $.html() )
        );
    },
    ...
    generateCode: function( values, targetHTMLFilePath ){
        ...
        //console.log( "values", targetHTMLFilePath );
        //console.log( values );
