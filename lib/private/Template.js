...
Template.prototype = {
    ...
    init: function(){
        ...
        fs.readFile( this.path, "utf-8", function( err, data ){
            ...
            //console.log( "***", this.id, "***" )
            //console.log( this.convertToTemplateFormat( data ) )
    ...
    convertToTemplateFormat: function( HTMLCode ){
        ...
        var rootInstanceTagExcludedHTMLCode =
                HTMLCode
                    .replace( /<!-- InstanceBegin template[^>]+ -->/ig, "" )
                    .replace( "<!-- InstanceEnd -->", "" ),
            $ = cheerio.load( utils.activateInstanceTags( rootInstanceTagExcludedHTMLCode ) ),
            processedHTMLCode;
        ...
        return _.reduce(
            {
                "<instanceeditable name=\"([^\"]+)\">[\\s\\S]*?</instanceeditable>":
